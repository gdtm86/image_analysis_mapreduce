      ...
      //Insert the return_buff byte[] in HBase table // //Create an instance of the Hbase table
        ...
        for(int i=0; i < 10; i++ ){
    	    ...
    	    UUID id = UUID.randomUUID();
          	...
          	put.add(Bytes.toBytes("image_metadata"), Bytes.toBytes("UserComment"),Bytes.toBytes(metadata.get("User Comment")));
          	put.add(Bytes.toBytes("image_metadata"),Bytes.toBytes("Content-Type"),Bytes.toBytes(metadata.get("Content-Type")));
          	put.add(Bytes.toBytes("image_metadata"), Bytes.toBytes("Image Width"),Bytes.toBytes(metadata.get("Exif Image Width")));
          	put.add(Bytes.toBytes("image_metadata"),Bytes.toBytes("Image Height"),Bytes.toBytes(metadata.get("Exif Image Height")));
